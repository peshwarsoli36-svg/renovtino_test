#!/usr/bin/env node
/**
 * Applies supabase/migration-staff.sql using a direct Postgres connection.
 *
  ...
  if (!password || !ref) {
    console.error(
      "Missing database credentials.\n\n" +
        ...
        "Or run supabase/migration-staff.sql manually in the Supabase SQL Editor."
    );
    process.exit(1);
  }
...
async function main() {
  ...
  const sql = readFileSync(
    resolve(process.cwd(), "supabase/migration-staff.sql"),
    "utf8"
  );
  ...
  try {
    ...
    process.stdout.write("Applied supabase/migration-staff.sql successfully.\n");
  } finally {
    await client.end();
  }
}
